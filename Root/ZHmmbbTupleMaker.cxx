...
EL::StatusCode ZHmmbbTupleMaker :: processEvent(){
  ...
  for (std::vector<int>::iterator it = list.begin() ; it != list.end(); ++it){
    if( !passMediumMuon( m_tuple->mm_leg1[*it], 0) ) continue;
    if( !passLooseMuon( m_tuple->mm_leg2[*it], 0) ) continue;
  ...
  // //Lepton isolation
  if(leptonIsoOption == 1){
    for (std::vector<int>::iterator it = list.begin() ; it != list.end(); ++it){
      if( m_tuple->muo_ptiso[m_tuple->mm_leg1[*it]]/m_tuple->muo_pt[m_tuple->mm_leg1[*it]] > 0.1  ) continue;
      if( m_tuple->muo_ptiso[m_tuple->mm_leg2[*it]]/m_tuple->muo_pt[m_tuple->mm_leg2[*it]] > 0.1  ) continue;
      listsel.push_back(*it);
    }
    if(listsel.size()==0) return EL::StatusCode::FAILURE;
    list=listsel;
  ...
  // //order by pT
  // if(m_tuple->muo_pt[l1] < m_tuple->muo_pt[l2]){
  //   int tmp=l1;
  //   l1 = l2;
  //   l2 = tmp; 
  // }
  ...
  //   listsel.push_back(*it);
  // }
  // if(listsel.size()<2) return EL::StatusCode::FAILURE;
  // list=listsel;
  // listsel.clear();
  ...
  // for (std::vector<int>::iterator it = list.begin() ; it != list.end(); ++it){
  //   if( m_tuple->muo_ptiso[*it]/m_tuple->muo_pt[*it] > 0.1  ) continue;
  //   listsel.push_back(*it);
  // }
  // if(listsel.size()<2) return EL::StatusCode::FAILURE;
  // list=listsel;
  ...
  // for (std::vector<int>::iterator it = list.begin() ; it != list.end(); ++it){
  //   if( m_tuple->muo_etiso[*it]/m_tuple->muo_pt[*it] > 0.1 ) continue;
  //   listsel.push_back(*it);
  // }
  // if(listsel.size()<2) return EL::StatusCode::FAILURE;
  // list=listsel;
  ...
  // if(list.size()>2) return EL::StatusCode::FAILURE;
