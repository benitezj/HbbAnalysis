...
ClassImp(WmunuTupleMaker)


WmunuTupleMaker :: WmunuTupleMaker ()
{
  ///Do not use this constructor
}

WmunuTupleMaker :: WmunuTupleMaker (std::string configPath) :
  WlnuTupleMaker(configPath),
  m_tuple(0)
{
}

EL::StatusCode WmunuTupleMaker :: initialize ()
{

  m_tuple = new WmunuTuple();
  WlnuTupleMaker::m_tuple = m_tuple ;

  return WlnuTupleMaker::initialize(); 
}

EL::StatusCode WmunuTupleMaker :: processEvent ()
{
  ...
  //init loop
  for(int i=0;i<m_tuple->nmuMet;i++){
    list.push_back(i);
  }
  if(list.size()==0) return EL::StatusCode::FAILURE;
  incrementCounter("eventCounter_nmuMet");
  ...
  //leg1 et iso cut
  for (std::vector<int>::iterator it = list.begin() ; it != list.end(); ++it){
    if( m_tuple->muo_etiso[*it]/m_tuple->muo_pt[*it] > 0.07 ) continue;
    listsel.push_back(*it);
  }
  if(listsel.size()==0) return EL::StatusCode::FAILURE;
  list=listsel;
  ...
  l1p4.SetPtEtaPhiM(m_tuple->muo_pt[m_tuple->wlnu_lMet],
		    m_tuple->muo_eta[m_tuple->wlnu_lMet],
		    m_tuple->muo_phi[m_tuple->wlnu_lMet],
		    m_tuple->muo_m[m_tuple->wlnu_lMet]);
